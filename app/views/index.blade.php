			...
			<div class="volleyball-division col-md-4">
				<div class="panel panel-default">
					<div class="panel-heading">
						<h3 class="panel-title">{{ $division->name }} Division</h3>
					</div>
					<div class="panel-body">
						<div class="volleyball-division-details">
							<p>{{ $division->description }}</p>
							@if ($division->allow_team_registration && $division->allow_solo_registration && $division->allow_additional_team_member)
							...
							@elseif ($division->allow_team_registration && $division->allow_solo_registration)
								<span class="emphasis">${{ $division->formatted_team_price }}</span> for up to {{ $division->min_team_members }} people
								<br /><span class="emphasis">${{ $division->formatted_solo_price }}</span> per person without team
							@elseif ($division->allow_team_registration && $division->allow_additional_team_member)
								<span class="emphasis">${{ $division->formatted_team_price }}</span> for up to {{ $division->min_team_members }} people, <span class="emphasis">${{ $division->formatted_additional_team_member_price }}</span> per additional
							@elseif ($division->allow_solo_registration && $division->allow_additional_team_member)
								@if ($division->formatted_solo_price == $division->formatted_additional_team_member_price)
									<span class="emphasis">${{ $division->formatted_solo_price }}</span> per person
								@else
									<span class="emphasis">${{ $division->formatted_solo_price }}</span> per person without team
									<br /><span class="emphasis">${{ $division->formatted_additional_team_member_price }}</span> per additional player on an existing team
								@endif
